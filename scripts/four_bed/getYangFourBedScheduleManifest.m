...
%GETYANGFOURBEDSCHEDULEMANIFEST Build the Yang four-bed schedule manifest.
%
...
% does not call the solver, define pair identities, or create physical state.
    ...
    executableDurationUnits = durationUnitsT24;
    executableDurationFractions = normalizedFractionOfDisplayedCycle;
    executableStart = normalizedStart;
    executableEnd = normalizedEnd;
    ...
    sourceColumns = table( ...
        ...
        executableDurationUnits, ...
        executableDurationFractions, ...
        rawStartUnitsT24, ...
        rawEndUnitsT24, ...
        normalizedStart, ...
        normalizedEnd, ...
        ...
        'VariableNames', [
            ...
            "executable_duration_units"
            "executable_duration_fraction"
            "raw_start_units_t24"
            "raw_end_units_t24"
            "normalized_start"
            "normalized_end"
    ...
    manifest = struct();
    manifest.version = "FI1-Yang2009-Table2-normalized-v1";
    ...
    manifest.duration = struct( ...
        ...
        "rawFractionPolicy", "metadata_only_not_executable_cycle_time_mapping", ...
        "executableUnits", executableDurationUnits, ...
        "executableUnitsPerCycle", rawUnitsPerDisplayedCycle, ...
        "executableFractions", executableDurationFractions, ...
        ...
        "durationHelper", "getYangNormalizedSlotDurations(cycleTimeSec)" ...
    );
...
function bedSteps = buildBedSteps(sourceColumns, bedLabels, glossary)
    ...
    executableDurationUnits = zeros(nRows, 1);
    executableDurationFraction = zeros(nRows, 1);
    rawStartUnitsT24 = zeros(nRows, 1);
    rawEndUnitsT24 = zeros(nRows, 1);
    executableStart = zeros(nRows, 1);
    executableEnd = zeros(nRows, 1);
    ...
    for sourceIdx = 1:height(sourceColumns)
        for bedIdx = 1:numel(bedLabels)
            ...
            executableDurationUnits(row) = sourceColumns.executable_duration_units(sourceIdx);
            executableDurationFraction(row) = sourceColumns.executable_duration_fraction(sourceIdx);
            ...
            executableStart(row) = sourceColumns.executable_start(sourceIdx);
            executableEnd(row) = sourceColumns.executable_end(sourceIdx);
